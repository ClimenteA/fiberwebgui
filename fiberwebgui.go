...
func startBrowser(guiWg *sync.WaitGroup, browserClosed chan bool, browserPath, port string, windowSizeFlag string, appMode bool) {
	...
	if appMode {
		...
		cmd := exec.Command(browserExecPath, url)
		err := cmd.Run()
		if err != nil {
			log.Fatal(err)
		}
...
func run(app FiberApp, windowSizeFlag string, port int, appMode bool) {
	...
	guiWg.Add(2)
	go startBrowser(&guiWg, browserClosed, browserPath, portstr, windowSizeFlag, appMode)
...
func Run(app FiberApp) {
	port := getFreePort()
	run(app, "--start-maximized", port, true)
}
...
func RunBrowser(app FiberApp) {
	port := getFreePort()
	run(app, "", port, false)
}

func RunBrowserOnPort(app FiberApp, port int) {
	run(app, "", port, false)
}
